...
class Index extends Controller
{
    ...
    public function index()
    {
        ...
        $rule = $addonCfg['types'];
        $max = intval(max($rule)) ;
        $max_key = array_search(max($rule), $rule);
        $rule['n'] = $max;
        //当天
        ...
        $sign = $this->model->where($where)->order('sign_time desc')->find();
        if($sign){
            if ($addonCfg['signstatus'] == 1)
            {
                $max_sign_num = $sign['max_sign'] + 1;
                //当月的第一天
    ...
    public function getdayList(){
        ...
        {
            if (date("Y-m",$time) == substr($v['sign_time'], 0, 7))
            {
                $days[] =  substr($v['sign_time'], 0, 10);
            }
        }
        $this->success('','',$days);
    }
