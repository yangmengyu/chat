...
class Api extends Controller
{
    ...
    public function create()
    {
        ...
        $content = $this->request->request('content');
        $dynamic_id = $this->request->request('key');
        ...
        if ($this->config['sensitive']) {
            // 敏感词过滤
            $wordFilePath = ADDON_PATH . 'comment/data/words.txt';
        ...
        if ($pid && $this->config['showtype'] == 'reply') {
            ...
            $comment['content'] = "<blockquote><a href='" . $parentComment['userinfo']['url'] . "' target='_blank'>@" . $parentComment['userinfo']['nickname'] . "</a> " . mb_substr($parentComment['content'], 0, 100) . "</blockquote>" . $comment['content'];
        }
        ...
        if ($this->article) {
            ...
            foreach ($commentData['data'] as $index => $datum) {
                ...
                if ($this->config['markdown']) {
                    $datum['content'] = Markdown::text($datum['content']);
                }
                ...
                if ($this->config['showtype'] == 'floor') {
                    ...
                    foreach ($commentList as $index => &$item) {
                        if ($item['pid']) {
                            ...
                            if ($parentComment) {
                                $item['content'] = "<blockquote><a href='" . $parentComment['userinfo']['url'] . "' target='_blank'>@" . $parentComment['userinfo']['nickname'] . "</a> " . mb_substr($parentComment['content'], 0, 100) . "</blockquote>" . $item['content'];
                            }
                        }
                    }
                    unset($item);
    ...
    public function login()
    {
        ...
        if ($type == 'account') {
            $url = url("index/user/login") . '?url=' . urlencode($this->request->url());
        } else {
            $url = "/third/connect/{$type}" . '?url=' . urlencode($this->request->url());
        }
        $this->redirect($url);
    }
