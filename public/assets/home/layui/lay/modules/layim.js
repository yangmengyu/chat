...
layui.define(['layer', 'laytpl', 'upload'], function(exports){
    ...
    var elemTpl = ['<div class="layui-layim-main">'
        ...
        ,'<li>'
        ,'<h5 layim-event="spread" lay-type="{{ spread }}"  data-groupid="{{item.id}}"  data-groupname="{{item.groupname}}"><i class="layui-icon">{{# if(spread === "true"){ }}&#xe61a;{{# } else {  }}&#xe602;{{# } }}</i><span>{{ item.groupname||"未命名分组"+index }}</span><em>(<cite class="layim-count"> {{ (item.list||[]).length }}</cite>)</em></h5>'
    ...
    var layimClose, popmin = function(content){
        ...
        return layer.open({
            ...
            ,offset: 'rb'
            ,resize: false
            ,content: '<img src="'+ (cache.mine.avatar||('/assets/home/img/default_avatar.jpg')) +'"><span>'+ (content||cache.base.title||'我的LayIM') +'</span>'
    ...
    var sendMessage = function(){
        var data = {
            ...
            ,avatar: cache.mine ? cache.mine.avatar : ('/assets/home/img/default_avatar.jpg')
