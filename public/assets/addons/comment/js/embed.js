...
;(function () {
    ...
    var S = window.S = {
        ...
        BUTTONS: {
            ...
            'emoji': '<a class="ds-toolbar-button ds-add-emote" title="插入表情"></a>',
        ...
        TEMPLATES: {
            'comment': '<li class="ds-post" data-post-id="<%=comment.id%>" data-pid="<%=comment.pid%>" data-level="<%=level%>" data-nickname="<%=comment.userinfo.nickname%>">\
                            ...
                            <div class="ds-comment-body">\
                                ...
                                <div class="ds-comment-footer ds-comment-actions">\
                                    ...
                                    <a class="ds-post-likes" href="javascript:void(0);">\
                                        <span class="ds-icon ds-icon-like"></span>赞<%=comment.likes>0?"("+comment.likes+")":""%>\
                                    </a>\
                        ...
                        <ul class="ds-children">\
                            ...
                            <div class="ds-post-more"><a href="javascript:;" data-page="2" data-pid="<%=comment.id%>" data-level="<%=level+1%>">加载更多</a></div>\
                            <%}%>\
                        </ul>\
                        <%}%></li>',
            'commentinfo': '<div class="ds-sort">\
									<a class="ds-order-desc <%=S.CONFIG.sort=="new"?"ds-current":""%>">最新</a>\
									<a class="ds-order-asc <%=S.CONFIG.sort=="early"?"ds-current":""%>">最早</a>\
									<a class="ds-order-hot <%=S.CONFIG.sort=="hot"?"ds-current":""%>">最热</a>\
								</div>\
								<ul class="ds-comments-tabs">\
									<li class="ds-tab">\
										<a class="ds-comments-tab-duoshuo ds-current" href="javascript:void(0);"><%=total%>条评论</a>\
									</li>\
								</ul>',
                            ...
                            <div class="ds-dialog-inner ds-rounded">\
                                <div class="ds-dialog-body">\
                                    ...
                                    <form methd="post" action="<%=S.PATH.report%>" class="form-report">\
                                        ...
                                        <label><input type="radio" name="type" value="1" checked> 色情低俗</label>\
                                        <label><input type="radio" name="type" value="2"> 政治敏感</label>\
                                        <label><input type="radio" name="type" value="3"> 违法暴力</label>\
                                        <label><input type="radio" name="type" value="4"> 恐怖血腥</label>\
                                        <label><input type="radio" name="type" value="5"> 非法贩卖</label>\
                                        <label><input type="radio" name="type" value="6"> 仇恨言论</label>\
                                        <label><input type="radio" name="type" value="7"> 打小广告</label>\
                                        <label><input type="radio" name="type" value="8"> 其他</label>\
                                        <textarea name="content" placeholder="说明详细情况"></textarea><br>\
                                ...
                                <a class="ds-dialog-close" href="javascript:void(0)" title="关闭"></a>\
                            </div>\
                        </div>',
            'postform': '<a class="ds-avatar" href="<%=S.USERINFO.url%>" target="_blank" title="设置头像">\
                            ...
                            <div class="ds-post-toolbar">\
                                <div class="ds-post-options ds-gradient-bg">\
                                    ...
                                    <span class="ds-sync">\
                                        ...
                                        <a href="javascript:void(0)" class="ds-service-icon-grey ds-qqt" data-service="qqt" title="腾讯微博"></a>\
                                        <a href="javascript:void(0)" class="ds-service-icon-grey ds-qzone" data-service="qzone" title="QQ空间"></a>\
                                    </span>\
                                    <%}%>\
                                </div>\
                                <button class="ds-post-button ajax-post" type="button" target-form="comment-form">发布</button>\
            ...
            'replyform': '<a class="ds-avatar" href="<%=S.USERINFO.url%>" target="_blank" title="设置头像">\
                            <img src="<%=S.USERINFO.avatar%>" alt="<%=S.USERINFO.nickname%>">\
                        </a>\
                        <form method="post" class="ds-form comment-form" action="<%=S.PATH.submit%>">\
                            <input type="hidden" name="key" value="<%=S.THREAD.key%>">\
                        ...
                        <div class="ds-dialog-inner ds-rounded">\
                            <div class="ds-dialog-body">\
                                ...
                                <div class="ds-icons-32">\
                                    <%if(S.CONFIG.thirdLogin.indexOf("account")>-1){%><a class="ds-modal-login ds-account" href="<%=S.APIURL%>login?type=account&name=<%=S.NAME%>">账户登录</a><%}%>\
        ...
        replyTips: '你的内容已经提交，等待审核通过后便可显示！',
        init: function () {
            template.helper("S", S);
            S.setPath();
            S.setContainer();
                        ...
                        if (ret.data.totalCount > 0) {
                            ...
                            if (S.CONFIG.nextPage == 'page') {
                                ...
                                if (ret.data.lastPage > ret.data.page) {
                                    //加载更多的方式
                                    var loadMore = $("<div />").addClass("ds-more").html('<a href="javascript:;" data-page="2" data-level="2">加载更多</a>')
                                    S.DOMS.page = loadMore;
                                }
                            }
                        } else {
                            S.DOMS.comment.html('<li class="ds-post ds-post-placeholder">还没有评论，沙发等你来抢</li>');
                        }
                    }
        ...
        createPower: function () {
            var dom = $("<div />").addClass("ds-powered-by").html();
            S.DOMS.power = dom;
        },
        ...
        dom.on("click", "[type=submit]", function () {
            ...
            codeAjax({
                ...
                success: function (data) {
                    ...
                    $(that).prop("disabled", false).html("提交反馈");
                }
            });
        });
    }
            ...
            success: function (ret) {
                if (ret.code == 1) {
                    ...
                    if (ret.data.datas.length > 0) {
                        ...
                        html = '<li class="ds-post ds-post-placeholder">已经没有更多</li>';
                    }
